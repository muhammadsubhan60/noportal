...
interface UserRow { id: string; firstName: string; lastName: string; email: string; isActive: boolean; }
interface VendorItem {
  vendorId: string; vendorName: string; carrier: string;
  vendorType: string; shippingService: string; baseRate: number; portal: string;
}
...
const ResellerBulkAccess: React.FC = () => {
  ...
  useEffect(() => {
    setLoadingU(true);
    axios.get('/users/reseller/clients')
      .then(r => setClients((r.data.clients || []).map((c: any) => ({
        id: c._id || c.id, firstName: c.firstName, lastName: c.lastName,
        email: c.email, isActive: c.isActive,
      }))))
      .catch(() => {})
  ...
  useEffect(() => {
    ...
    axios.get('/access/me')
      .then(r => {
        const list: VendorItem[] = (r.data.access as any[]).map(v => ({
  ...
  const filteredClients  = clients.filter(c => !userSearch || `${c.firstName} ${c.lastName} ${c.email}`.toLowerCase().includes(userSearch.toLowerCase()));
  const filteredVendors  = vendors.filter(v => !vendorSearch || `${v.vendorName} ${v.shippingService}`.toLowerCase().includes(vendorSearch.toLowerCase()));
  const allClientsChecked  = filteredClients.length > 0 && filteredClients.every(c => selectedU.has(c.id));
  const allVendorsChecked  = filteredVendors.length > 0 && filteredVendors.every(v => selectedV.has(v.vendorId));

  const toggleClient = (id: string) => setSelectedU(s => { const n = new Set(s); n.has(id) ? n.delete(id) : n.add(id); return n; });
  const toggleVendor = (id: string) => setSelectedV(s => { const n = new Set(s); n.has(id) ? n.delete(id) : n.add(id); return n; });
  const toggleAllClients = () => {
    if (allClientsChecked) setSelectedU(s => { const n = new Set(s); filteredClients.forEach(c => n.delete(c.id)); return n; });
    else                   setSelectedU(s => { const n = new Set(s); filteredClients.forEach(c => n.add(c.id));  return n; });
  };
  const toggleAllVendors = () => {
    if (allVendorsChecked) setSelectedV(s => { const n = new Set(s); filteredVendors.forEach(v => n.delete(v.vendorId)); return n; });
    else                   setSelectedV(s => { const n = new Set(s); filteredVendors.forEach(v => n.add(v.vendorId));    return n; });
  };
  ...
  const applyAccess = async (isAllowed: boolean) => {
    ...
    try {
      ...
      await axios.put('/access/bulk/vendor-access', { userIds: Array.from(selectedU), vendorEntries, isAllowed });
      setMessage(`${isAllowed ? 'Enabled' : 'Disabled'} ${vendorEntries.length} vendor(s) for ${selectedU.size} client(s)`);
    } catch (err: any) {
  ...
  const openRateModal = () => {
    setFlatRate(''); setTiers([{ minLbs: '0', maxLbs: '', rate: '' }]); setRateMode('replace'); setShowRateModal(true);
  };
  const handleFlatRate = (val: string) => { setFlatRate(val); setTiers([{ minLbs: '0', maxLbs: '', rate: val }]); };
  const updateTier = (i: number, field: string, val: string) => setTiers(ts => ts.map((t, idx) => idx === i ? { ...t, [field]: val } : t));
  ...
  const applyRates = async () => {
    ...
    const validTiers = tiers.filter(t => t.rate !== '').map(t => ({
    ...
    try {
      ...
      const result = await axios.put('/access/bulk/rates', {
        userIds: Array.from(selectedU), vendorEntries, rateTiers: validTiers, mode: rateMode,
      });
      setMessage(result.data.message);
      setShowRateModal(false);
    } catch (err: any) {
      setError(err.response?.data?.message || 'Failed to apply rates');
    } finally { setApplyingRates(false); }
  };
  ...
  return (
    <div style={{ fontFamily: FONT, display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 7 }}>
            <button onClick={() => navigate('/reseller/clients')}
              ...
              <ArrowLeftIcon style={{ width: 13, height: 13 }} /> Clients
            </button>
            ...
            <span style={{ fontSize: '0.6rem', fontWeight: 700, color: '#6366f1', textTransform: 'uppercase', letterSpacing: '0.1em', background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.2)', padding: '2px 7px', borderRadius: 99, fontFamily: FONT }}>Reseller</span>
          </div>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {[
              { label: 'Clients',          value: clients.length,   color: 'var(--navy-600)', bg: 'var(--navy-50)',        border: 'var(--navy-200)'           },
              { label: 'Vendors',          value: vendors.length,   color: '#6366f1',         bg: 'rgba(99,102,241,0.07)', border: 'rgba(99,102,241,0.2)'       },
              { label: 'Selected Clients', value: selectedU.size,   color: '#10b981',         bg: 'rgba(16,185,129,0.07)', border: 'rgba(16,185,129,0.2)'       },
              { label: 'Selected Vendors', value: selectedV.size,   color: '#f59e0b',         bg: 'rgba(245,158,11,0.07)', border: 'rgba(245,158,11,0.2)'       },
      ...
      {/* Toast */}
      {(message || error) && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '0.65rem 1rem', borderRadius: 10, fontFamily: FONT, fontSize: '0.82rem', fontWeight: 600, background: message ? 'rgba(16,185,129,0.08)' : 'rgba(239,68,68,0.08)', border: `1px solid ${message ? 'rgba(16,185,129,0.25)' : 'rgba(239,68,68,0.25)'}`, color: message ? '#10b981' : '#ef4444' }}>
      ...
      {(selectedU.size > 0 || selectedV.size > 0) && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10, padding: '0.7rem 1.1rem', borderRadius: 12, fontFamily: FONT, background: 'linear-gradient(135deg,rgba(99,102,241,0.07),rgba(99,102,241,0.02))', border: '1.5px solid rgba(99,102,241,0.18)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
              ...
              <span style={{ fontSize: '0.8rem', fontWeight: 700, color: selectedU.size > 0 ? 'var(--navy-800)' : 'var(--navy-400)', fontFamily: FONT }}>
                {selectedU.size > 0 ? `${selectedU.size} client${selectedU.size !== 1 ? 's' : ''}` : 'No clients selected'}
              </span>
            </div>
          ...
          <div style={{ display: 'flex', gap: 7 }}>
            <button onClick={() => applyAccess(true)} disabled={!canApply} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0.5rem 1.1rem', borderRadius: 8, border: 'none', background: canApply ? 'linear-gradient(135deg,#10b981,#059669)' : 'var(--navy-100)', color: canApply ? '#fff' : 'var(--navy-400)', fontSize: '0.82rem', fontWeight: 700, cursor: canApply ? 'pointer' : 'not-allowed', fontFamily: FONT, boxShadow: canApply ? '0 4px 12px rgba(16,185,129,0.3)' : 'none', transition: 'all 0.15s', opacity: applying ? 0.7 : 1 }}>
              <ShieldCheckIcon style={{ width: 14, height: 14 }} />
              {applying ? 'Applying…' : `Enable for ${selectedU.size || '—'}`}
            </button>
            <button onClick={() => applyAccess(false)} disabled={!canApply} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0.5rem 1.1rem', borderRadius: 8, border: `1.5px solid ${canApply ? 'rgba(239,68,68,0.3)' : 'var(--navy-100)'}`, background: canApply ? 'rgba(239,68,68,0.07)' : 'transparent', color: canApply ? '#ef4444' : 'var(--navy-400)', fontSize: '0.82rem', fontWeight: 700, cursor: canApply ? 'pointer' : 'not-allowed', fontFamily: FONT, transition: 'all 0.15s', opacity: applying ? 0.7 : 1 }}>
              <ShieldExclamationIcon style={{ width: 14, height: 14 }} />
              {applying ? 'Applying…' : `Disable for ${selectedU.size || '—'}`}
            </button>
            <button onClick={openRateModal} disabled={!canApply} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0.5rem 1.1rem', borderRadius: 8, border: 'none', background: canApply ? 'linear-gradient(135deg,#6366f1,#4f46e5)' : 'var(--navy-100)', color: canApply ? '#fff' : 'var(--navy-400)', fontSize: '0.82rem', fontWeight: 700, cursor: canApply ? 'pointer' : 'not-allowed', fontFamily: FONT, boxShadow: canApply ? '0 4px 12px rgba(99,102,241,0.35)' : 'none', transition: 'all 0.15s' }}>
        ...
        <div className="db-card" style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', maxHeight: 'calc(100vh - 220px)' }}>
          <div style={{ padding: '0.8rem', borderBottom: '1px solid var(--navy-100)', display: 'flex', flexDirection: 'column', gap: 7 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <span style={{ fontSize: '0.68rem', fontWeight: 700, color: 'var(--navy-500)', textTransform: 'uppercase', letterSpacing: '0.09em', fontFamily: FONT }}>My Clients</span>
              </div>
              {selectedU.size > 0 && (
                <button onClick={() => setSelectedU(new Set())} style={{ fontSize: '0.67rem', fontWeight: 600, color: '#6366f1', background: 'none', border: 'none', cursor: 'pointer', fontFamily: FONT }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input type="text" style={{ ...inp, paddingLeft: '1.85rem', fontSize: '0.78rem' }}
                placeholder="Search clients…" value={userSearch}
                onChange={e => setUserSearch(e.target.value)} onFocus={focusI} onBlur={blurI} />
            </div>
          </div>

          {/* Select-all */}
          <div onClick={toggleAllClients} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0.45rem 0.8rem', borderBottom: '1px solid var(--navy-50)', cursor: 'pointer', background: allClientsChecked ? 'rgba(99,102,241,0.04)' : 'transparent' }}>
            <input type="checkbox"
              ref={el => { if (el) el.indeterminate = !allClientsChecked && filteredClients.some(c => selectedU.has(c.id)); }}
              checked={allClientsChecked} onChange={toggleAllClients} onClick={e => e.stopPropagation()} style={{ cursor: 'pointer' }} />
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0.35rem' }}>
            {loadingU ? (
              ...
              <div style={{ textAlign: 'center', padding: '2rem 1rem', color: 'var(--navy-400)', fontSize: '0.8rem', fontFamily: FONT }}>No clients found</div>
            ) : filteredClients.map(c => {
              const isSelected = selectedU.has(c.id);
              return (
                <div key={c.id} onClick={() => toggleClient(c.id)}
                  style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0.45rem 0.55rem', borderRadius: 8, cursor: 'pointer', marginBottom: 2, background: isSelected ? 'rgba(99,102,241,0.07)' : 'transparent', border: `1.5px solid ${isSelected ? 'rgba(99,102,241,0.2)' : 'transparent'}`, borderLeft: `2.5px solid ${isSelected ? '#6366f1' : 'transparent'}`, transition: 'all 0.1s' }}>
                  <input type="checkbox" checked={isSelected} onChange={() => toggleClient(c.id)} onClick={e => e.stopPropagation()} style={{ cursor: 'pointer', flexShrink: 0 }} />
                  <div style={{ width: 28, height: 28, borderRadius: '50%', flexShrink: 0, background: c.isActive ? 'linear-gradient(135deg,#6366f1,#4f46e5)' : 'linear-gradient(135deg,#94a3b8,#64748b)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.65rem', fontWeight: 700, color: '#fff', fontFamily: FONT }}>
                    {c.firstName[0]}{c.lastName[0]}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: '0.77rem', fontWeight: 600, color: 'var(--navy-900)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', fontFamily: FONT }}>{c.firstName} {c.lastName}</div>
                    <div style={{ fontSize: '0.65rem', color: 'var(--navy-400)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', fontFamily: FONT }}>{c.email}</div>
                  </div>
                  <div style={{ width: 5, height: 5, borderRadius: '50%', background: c.isActive ? '#10b981' : '#ef4444', flexShrink: 0 }} />
                </div>
              );
            })}
          </div>

          <div style={{ borderTop: '1px solid var(--navy-100)', padding: '0.45rem 0.8rem' }}>
            <span style={{ fontSize: '0.67rem', color: 'var(--navy-400)', fontFamily: FONT }}>{clients.length} client{clients.length !== 1 ? 's' : ''}</span>
          </div>
        </div>
          ...
          <div style={{ padding: '0.8rem', borderBottom: '1px solid var(--navy-100)', display: 'flex', flexDirection: 'column', gap: 7 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                {selectedV.size > 0 && (
                  <button onClick={() => setSelectedV(new Set())} style={{ fontSize: '0.67rem', fontWeight: 600, color: '#6366f1', background: 'none', border: 'none', cursor: 'pointer', fontFamily: FONT }}>
                    Clear {selectedV.size}
                  </button>
                )}
                <button onClick={toggleAllVendors} style={{ fontSize: '0.67rem', fontWeight: 600, color: 'var(--navy-600)', background: 'var(--navy-50)', border: '1px solid var(--navy-200)', borderRadius: 5, padding: '2px 8px', cursor: 'pointer', fontFamily: FONT }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input type="text" style={{ ...inp, paddingLeft: '1.85rem', fontSize: '0.78rem' }}
                ...
                onChange={e => setVendorSearch(e.target.value)} onFocus={focusI} onBlur={blurI} />
            </div>
          </div>
              ...
              CARRIERS_ORDER.map(carrier => {
                ...
                return (
                  <div key={carrier} style={{ border: `1.5px solid ${cfg.border}`, borderRadius: 10, overflow: 'hidden', marginBottom: 8 }}>
                    <div onClick={() => setOpenCarriers(o => ({ ...o, [carrier]: !o[carrier] }))}
                      ...
                      {isOpen ? <ChevronUpIcon style={{ width: 13, height: 13, color: 'var(--navy-400)', flexShrink: 0 }} />
                              ...
                              {pvs.map((v, vi) => {
                                ...
                                return (
                                  <div key={v.vendorId} onClick={() => toggleVendor(v.vendorId)}
                                    style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0.4rem 0.875rem 0.4rem 1rem', cursor: 'pointer', borderTop: vi === 0 ? 'none' : '1px solid var(--navy-50)', background: isSel ? 'rgba(99,102,241,0.04)' : 'transparent', transition: 'background 0.1s' }}>
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                      ...
                                      {v.shippingService && <span style={{ marginLeft: 6, fontSize: '0.65rem', color: 'var(--navy-400)', fontFamily: FONT }}>{v.shippingService}</span>}
                                    </div>
      ...
      {showRateModal && ReactDOM.createPortal(
        <div onClick={e => { if (e.target === e.currentTarget) setShowRateModal(false); }}
          ...
          <div style={{ background: 'var(--bg-card)', borderRadius: 16, border: '1.5px solid var(--navy-200)', boxShadow: 'var(--shadow-lg)', width: '100%', maxWidth: 500, margin: '1rem', maxHeight: '90vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid var(--navy-100)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                ...
                <div style={{ fontSize: '0.7rem', color: 'var(--navy-400)', marginTop: 2, fontFamily: FONT }}>
                  {selectedU.size} client{selectedU.size !== 1 ? 's' : ''} × {selectedV.size} vendor{selectedV.size !== 1 ? 's' : ''} = {selectedU.size * selectedV.size} records
                </div>
              </div>
            ...
            <div style={{ padding: '1rem 1.25rem', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <input type="number" min="0" step="0.01" placeholder="e.g. 0.40" value={flatRate}
                    onChange={e => handleFlatRate(e.target.value)}
                    style={{ ...inp, width: 140, fontSize: '0.88rem', fontWeight: 700 }} onFocus={focusI} onBlur={blurI} />
              ...
              <div>
                ...
                {tiers.map((t, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 32px', gap: 6, marginBottom: 5 }}>
                    <input type="number" min="0" step="0.1" placeholder="0" value={t.minLbs} onChange={e => { setFlatRate(''); updateTier(i, 'minLbs', e.target.value); }} style={{ ...inp, fontSize: '0.8rem' }} onFocus={focusI} onBlur={blurI} />
                    <input type="number" min="0" step="0.1" placeholder="∞" value={t.maxLbs} onChange={e => { setFlatRate(''); updateTier(i, 'maxLbs', e.target.value); }} style={{ ...inp, fontSize: '0.8rem' }} onFocus={focusI} onBlur={blurI} />
                    <input type="number" min="0" step="0.01" placeholder="0.00" value={t.rate} onChange={e => { setFlatRate(''); updateTier(i, 'rate', e.target.value); }} style={{ ...inp, fontSize: '0.8rem' }} onFocus={focusI} onBlur={blurI} />
                    <button onClick={() => { setFlatRate(''); removeTier(i); }} disabled={tiers.length === 1}
                ...
                <button onClick={() => { setFlatRate(''); addTier(); }}
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 7 }}>
                  {([
                    { val: 'replace'       as const, label: 'Replace all',     desc: 'Overwrites existing tiers for every selected client' },
                    { val: 'skip_existing' as const, label: 'Skip with tiers', desc: 'Only updates clients who have no tiers yet'          },
                  ]).map(({ val, label, desc }) => (
                    <button key={val} onClick={() => setRateMode(val)}
                      style={{ flex: 1, padding: '0.55rem 0.75rem', borderRadius: 9, textAlign: 'left', cursor: 'pointer', border: `1.5px solid ${rateMode === val ? 'rgba(99,102,241,0.4)' : 'var(--navy-200)'}`, background: rateMode === val ? 'rgba(99,102,241,0.07)' : 'transparent', transition: 'all 0.12s' }}>
            ...
            <div style={{ padding: '0.875rem 1.25rem', borderTop: '1px solid var(--navy-100)', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 8 }}>
              <button onClick={() => setShowRateModal(false)} style={{ padding: '0.5rem 1rem', borderRadius: 8, border: '1.5px solid var(--navy-200)', background: 'transparent', color: 'var(--navy-600)', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer', fontFamily: FONT }}>
                Cancel
              </button>
              <button onClick={applyRates} disabled={applyingRates}
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0.5rem 1.25rem', borderRadius: 8, border: 'none', background: 'linear-gradient(135deg,#6366f1,#4f46e5)', color: '#fff', fontSize: '0.82rem', fontWeight: 700, cursor: applyingRates ? 'not-allowed' : 'pointer', fontFamily: FONT, opacity: applyingRates ? 0.7 : 1, boxShadow: '0 4px 12px rgba(99,102,241,0.35)' }}>
                <CurrencyDollarIcon style={{ width: 14, height: 14 }} />
...
export default ResellerBulkAccess;
