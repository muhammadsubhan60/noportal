...
// Slide-over drawer portalled to body
    ...
    <>
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 340, maxWidth: '100vw', boxSizing: 'border-box', background: 'var(--bg-card)', boxShadow: '-4px 0 32px rgba(15,23,42,0.18)', zIndex: 1001, display: 'flex', flexDirection: 'column', fontFamily: FONT }}>
        ...
        <div style={{ padding: '0.9rem 1.3rem', borderBottom: '1px solid var(--navy-100)', display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: '0.62rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', padding: '3px 9px', borderRadius: 99, background: rs.bg, color: rs.color, border: `1px solid ${rs.border}` }}>
            {user.role}
          </span>
        ...
        <div style={{ padding: '0.85rem 1.3rem', borderBottom: '1px solid var(--navy-100)' }}>
          <div style={{ fontSize: '0.6rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--navy-400)', marginBottom: '0.6rem' }}>
            Stay in the loop
          </div>
          <div style={{ fontSize: '0.68rem', color: 'var(--navy-500)', marginBottom: '0.75rem', lineHeight: 1.5 }}>
            This user receives shipping updates — every label, delivery, and alert — straight to their email and WhatsApp.
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: '0.55rem', padding: '0.5rem 0.7rem', borderRadius: 8, background: 'rgba(99,102,241,0.04)', border: '1px solid rgba(99,102,241,0.12)' }}>
            ...
            <span style={{ fontSize: '0.58rem', fontWeight: 700, color: '#22c55e', background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.2)', padding: '2px 7px', borderRadius: 99 }}>Active</span>
          </div>
            ...
            {editingPhone && (
              <div style={{ marginTop: '0.5rem' }}>
                <input
                  autoFocus
                  value={phoneVal}
                  onChange={e => { setPhoneVal(e.target.value); setPhoneErr(''); }}
...
export default function CCUsers() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API_BASE}/labels/user-stats-bulk`, { headers: authH(), params })
  ...
  const GRID = `36px ${COL} 84px 52px 66px 74px 68px 66px 54px 80px`;
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem', flexWrap: 'wrap', gap: 10 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
            ...
            {statsLoading && (
              <span style={{ fontSize: '0.65rem', color: 'var(--navy-400)', fontWeight: 500 }}>Loading stats…</span>
            )}
          </div>
      ...
      <div className="db-card" style={{ overflow: 'hidden' }}>
       <div style={{ overflowX: 'auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: GRID, gap: '0 0.75rem', alignItems: 'center', padding: '0.6rem 1rem', borderBottom: '1.5px solid var(--navy-100)', background: 'var(--navy-50)' }}>
          <div />
          <div onClick={() => handleSort('name')} style={thStyle('name')}>
            User <SortIcon active={sortKey === 'name'} dir={sortDir} />
          </div>
          <div style={{ fontSize: '0.62rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--navy-400)' }}>Role</div>
          <div style={{ fontSize: '0.62rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--navy-400)' }}>Status</div>
          <div onClick={() => handleSort('total')} style={{ ...thStyle('total'), justifyContent: 'flex-end' }}>
            Labels <SortIcon active={sortKey === 'total'} dir={sortDir} />
          </div>
          <div onClick={() => handleSort('deliveryRate')} style={{ ...thStyle('deliveryRate'), justifyContent: 'flex-end' }}>
            Delivery% <SortIcon active={sortKey === 'deliveryRate'} dir={sortDir} />
          </div>
          <div onClick={() => handleSort('scanRate')} style={{ ...thStyle('scanRate'), justifyContent: 'flex-end' }}>
            Scan% <SortIcon active={sortKey === 'scanRate'} dir={sortDir} />
          </div>
          <div onClick={() => handleSort('errorRate')} style={{ ...thStyle('errorRate'), justifyContent: 'flex-end' }}>
            Error% <SortIcon active={sortKey === 'errorRate'} dir={sortDir} />
          </div>
          <div onClick={() => handleSort('voided')} style={{ ...thStyle('voided'), justifyContent: 'flex-end' }}>
            Voided <SortIcon active={sortKey === 'voided'} dir={sortDir} />
          </div>
          <div onClick={() => handleSort('spent')} style={{ ...thStyle('spent'), justifyContent: 'flex-end' }}>
            Spent <SortIcon active={sortKey === 'spent'} dir={sortDir} />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--navy-400)', fontSize: '0.85rem' }}>No users found</div>
        ) : (
          sorted.map((u, idx) => {
            ...
            return (
              <div
                ...
                style={{
                  display: 'grid', gridTemplateColumns: GRID, gap: '0 0.75rem',
                  alignItems: 'center', padding: '0.65rem 1rem', cursor: 'pointer',
                  borderTop: idx === 0 ? 'none' : '1px solid var(--navy-100)',
              ...
              >
                {/* Avatar */}
                <div style={{ width: 32, height: 32, borderRadius: '50%', background: rs.avatarGrad, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '0.7rem', fontWeight: 700, flexShrink: 0 }}>
                  {initials || '?'}
                </div>
                ...
                {/* Role */}
                <div>
                  <span style={{ fontSize: '0.58rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', padding: '2px 7px', borderRadius: 99, background: rs.bg, color: rs.color, border: `1px solid ${rs.border}` }}>
                    {u.role}
                  </span>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  ...
                  <span style={{ fontSize: '0.68rem', color: u.isActive ? '#22c55e' : '#94a3b8', fontWeight: 600 }}>{u.isActive ? 'Active' : 'Off'}</span>
                </div>
                ...
                <div style={{ textAlign: 'right' }}>
                  {statsLoading && !stats ? (
                    ...
                    <span style={{ fontSize: '0.85rem', fontWeight: 800, color: stats?.total ? 'var(--navy-900)' : 'var(--navy-300)' }}>
                      {stats?.total ?? '—'}
                    </span>
                  )}
                </div>
                ...
                <div style={{ textAlign: 'right' }}>
                  {statsLoading && !stats ? (
                    ...
                    <span style={{ fontSize: '0.85rem', fontWeight: 800, color: deliveryRate >= 80 ? '#22c55e' : deliveryRate >= 50 ? '#f59e0b' : '#ef4444' }}>
                      {deliveryRate}%
                    </span>
                  ) : (
                ...
                <div style={{ textAlign: 'right' }}>
                  {statsLoading && !stats ? (
                    ...
                    <span style={{ fontSize: '0.85rem', fontWeight: 800, color: scanRate >= 80 ? '#22c55e' : scanRate >= 50 ? '#f59e0b' : '#ef4444' }}>
                      {scanRate}%
                    </span>
                  ) : (
                    <span style={{ fontSize: '0.85rem', color: 'var(--navy-300)' }}>—</span>
                ...
                <div style={{ textAlign: 'right' }}>
                  {statsLoading && !stats ? (
                    <div style={{ width: 32, height: 10, borderRadius: 4, background: 'var(--navy-100)', marginLeft: 'auto' }} />
                  ) : errorRate !== null ? (
                    <span style={{ fontSize: '0.85rem', fontWeight: 800, color: errorRate === 0 ? 'var(--navy-300)' : errorRate <= 5 ? '#f59e0b' : '#ef4444' }}>
                      {errorRate}%
                    </span>
                  ) : (
                    <span style={{ fontSize: '0.85rem', color: 'var(--navy-300)' }}>—</span>
                ...
                <div style={{ textAlign: 'right' }}>
                  {statsLoading && !stats ? (
                    <div style={{ width: 28, height: 10, borderRadius: 4, background: 'var(--navy-100)', marginLeft: 'auto' }} />
                  ) : (
                    <span style={{ fontSize: '0.85rem', fontWeight: 800, color: stats?.voided ? '#94a3b8' : 'var(--navy-300)' }}>
                      {stats?.voided ?? '—'}
                    </span>
                  )}
                </div>
                ...
                <div style={{ textAlign: 'right' }}>
                  {statsLoading && !stats ? (
                    <div style={{ width: 48, height: 10, borderRadius: 4, background: 'var(--navy-100)', marginLeft: 'auto' }} />
                  ) : (
                    <span style={{ fontSize: '0.85rem', fontWeight: 800, color: stats?.spent ? 'var(--navy-800)' : 'var(--navy-300)' }}>
                      {stats ? `$${stats.spent.toFixed(2)}` : '—'}
                    </span>
                  )}
                </div>
              </div>
